...
requests, so we used the provided \texttt{ip-wireshark-trace2-1.pcapng} trace
on a laptop running NixOS to complete the IPv6 questions. The capture contains
two back-to-back DNS queries for \texttt{youtube.com}. Both packets leave the
host \texttt{2601:193:8302:4620:215c:f5ae:8b40:a27a} towards the Comcast
resolver \texttt{2001:558:feed::1} with a Hop Limit of 255, which is the
typical value macOS uses on Wi-Fi. The flow labels are \texttt{0xa4918} and
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{img/2/ipv6_dns.png}
    \caption{AAAA exchange over IPv6}\label{fig:ipv6_dns}
\end{figure}
