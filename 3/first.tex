...
length possible is \(2^{16}\)-1-8 bytes, because the total length field of UDP
has only 16 bits, and the header size is 8 bytes. In a similar manner the
maximum UPD source port is \(2^{16}\)-1 because is a 16 bit field too. Finally,
UDP protocol number is 17 (0x11) by the way.
